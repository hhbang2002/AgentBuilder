...
| 문서 버전 | v0.3 |
| 작성일 | 2026-07-17 |
| 상태 | **확정** — 열린 질문 전체 답변 완료, 최종 리뷰 보완 반영 |
| 다음 단계 | 설계문서(02) 작성 → 개발 |
...
8. **배포별 구성 가능 (Deployment Profile)** — 플랫폼은 여러 고객사/사이트에 각각 설치되는
   것을 전제로 한다. 인증 방식(Keycloak 단독 vs 사내 IdP 연동), 외부 LLM API 허용 정책,
   커넥터 구성, GPU 프로파일 등 환경 의존 요소는 코드 변경 없이 **배포 설정 팩**으로
   전환 가능해야 한다.
...
#### FR-AGT-08 에이전트 컴포지션 (Agent-as-Tool) (P1)

- 배포된 에이전트를 다른 에이전트의 **도구로 등록**하여 호출할 수 있다 (예: "법규 Q&A
  에이전트"를 "위험성 평가 에이전트"가 하위 도구로 사용).
- 순환 호출 방지(호출 깊이 제한), 하위 에이전트 호출도 상위 트레이스에 중첩 기록.
- 하위 에이전트의 가드레일·권한 정책은 호출 경로와 무관하게 항상 적용된다.

#### FR-AGT-09 컨텍스트 윈도 관리 (P1)

- 장기 대화 시 컨텍스트 초과 방지: 오래된 턴 자동 요약 압축, 도구 결과 대용량 출력 절삭
  정책(요약 후 원문은 참조 링크로 유지).
- 노드별 컨텍스트 예산 설정(프롬프트/지식/이력 각각의 최대 토큰 비율).
...
- 배포 채널: REST API 엔드포인트 자동 생성(P0), 웹 챗 UI(P0), 임베더블 위젯(P1),
  ...
  이벤트 트리거 – 웹훅/Kafka 메시지 기반(P1), 배치 일괄 실행 – 대량 입력 파일 기반
  비동기 실행(예: 사고 보고서 1,000건 일괄 구조화)(P1).
...
#### FR-STD-08 에이전트 생성 마법사 (P1)

- 현업 사용자를 위한 단계형 생성 가이드: 과업 유형 선택(Q&A/분석/리포트/워크플로) →
  템플릿 추천 → 지식/도구 연결 → 지시문 초안 자동 생성(LLM 보조) → 테스트 → 골든셋
  작성 안내.
- 6.1의 입력 체크리스트(Readiness Check)와 통합되어 미비 항목을 단계 내에서 안내.
...
- 지원 포맷: PDF(스캔본 OCR 포함), Office(docx/xlsx/pptx), 이미지(도면 캡처 등), HTML,
  Markdown, 텍스트 (이상 P0). HWP/HWPX는 P1 — 대상 문서 중 비중이 낮은 것으로 확인되어
  MVP에서는 PDF 변환 우회 가이드로 대응하고, 정식 버전에서 네이티브 파서를 지원한다.
...
#### FR-KNW-08 법규/규정 문서 관리 (P1 — EHS 파일럿 우선 적용)

- 법규성 문서(법·시행령·고시, 사내 규정)의 **시행일/개정일/폐지 상태** 메타데이터 관리.
- 개정 발생 시: 신구 버전 병행 보관, 개정 전 버전 검색 시 "구버전" 경고 태깅, 해당 문서를
  인용 중인 에이전트 목록에 재평가 필요 알림 (FR-EVL-03 연계).
- 응답 인용 시 시행일 기준 유효 버전 표기 (예: "산업안전보건법 제38조, 2025-06-01 시행 기준").
- 법규 원문 자동 수집(국가법령정보센터 API 등)은 P2 — 폐쇄망 배포에서는 수동 반입 절차 제공.
...
- **배포별 외부 API 정책 (Deployment Profile)**: 외부 LLM API 허용 여부와 허용 범위는
  고객사마다 다르므로, 배포 설정 팩에서 `전면 금지 / 허용 목록 기반 부분 허용 / 전면 허용`
  을 선택하고 관리자가 코드 변경 없이 전환할 수 있어야 한다. 전면 금지 배포에서는 외부
  API 관련 UI/설정이 비노출된다.
...
#### FR-MDL-06 비전(멀티모달) 모델 지원 (P2)

- 이미지 입력을 지원하는 모델(로컬 VLM 또는 외부 API)을 게이트웨이 별칭 체계에 통합.
- 활용처: 현장 사진 위험 식별(EHS), 도면/라벨 판독, 불량 사진 분류 보조.
- 이미지에도 데이터 등급 정책(FR-MDL-02) 동일 적용 — 현장 사진의 외부 API 전송 통제.
...
- **연동 방식 선택형 설계**: 동일 시스템(MES/ERP/QMS/CMMS)이라도 DB 뷰 연동과 API 연동을
  모두 지원하며, 에이전트 개발자가 Agent 개발 시 대상 시스템·연동 방식(DB 뷰 vs API)을
  선택해 바인딩한다. 배포 설정 팩에 사전 등록된 커넥터 인스턴스 중에서 선택하는 구조로,
  자격증명·엔드포인트는 개발자에게 노출되지 않는다.
...
- **휴먼 평가 모드** (P1): 자동 평가로 판정이 어려운 케이스에 대해 도메인 전문가가
  블라인드 채점(버전 정보 가린 상태로 응답 비교/점수 부여)하는 UI. 채점 결과는
  LLM-Judge 신뢰도 교정에도 사용.
...
- 인증 프로파일 선택형: 배포 설정 팩에서 `Keycloak 단독(내장 사용자 관리)` 또는
  `사내 IdP 연동(OIDC/SAML 브로커링, LDAP/AD 동기화)`을 선택한다. 고객사의 SSO/AD
  보유 여부에 따라 Agent 적용 대상 조직 단위로 구성을 전환할 수 있어야 한다.
  (Keycloak 단독 P0, 사내 IdP 브로커링 P0, LDAP/AD 동기화 P1)
...
- **간접 인젝션 방어**: RAG로 검색된 문서·도구 결과 등 외부 유래 컨텍스트에 포함된 지시성
  텍스트를 데이터로 취급하도록 격리(구조적 구분 + 탐지 필터). 문서가 에이전트의 행동을
  바꾸도록 유도하는 패턴을 차단한다.
...
- 도메인 안전 규칙: 안전 관련 질의(설비 조작, 화학물질 취급 등)에는 반드시 공식 절차 문서
  인용 + 면책 고지를 포함하도록 강제하는 규칙 템플릿 (**P0로 상향** — EHS 파일럿 필수 요건).
...
- **배포 설정 팩(Deployment Profile)**: 인증 프로파일(FR-GOV-01), 외부 API 정책
  (FR-MDL-02), 커넥터 인스턴스 목록(FR-TOL-03), 모델 별칭 매핑, GPU 프로파일, 브랜딩을
  하나의 설정 번들로 관리한다. 고객사별 설치 시 설정 팩 교체만으로 환경을 구성하며,
  설정 팩 자체를 버전 관리한다.
- **개발/레퍼런스 GPU 프로파일**: RTX PRO 6000(96GB VRAM) 단일 장착 서버를 표준 개발·
  파일럿 프로파일로 정의한다. 해당 사양에서 30B급 모델(양자화 시 70B급) 서빙 + 임베딩/
  리랭커 동시 운용을 기준 구성으로 하고, 용량 산정 가이드를 제공한다.
...
#### FR-DEP-04 유량 제어 및 캐싱 (P2)

- API 레이트 리밋: API 키/사용자/에이전트 단위 요청 한도 (GPU 자원 보호).
- 시맨틱 캐시: 유사 질의에 대한 응답 재사용 옵션(에이전트별 on/off, TTL) — FAQ성
  트래픽의 GPU 부하 절감. 지식 갱신 시 관련 캐시 무효화.
...
**(1차·파일럿: 안전환경보건(EHS) → 2차: 품질·설비 → 3차: 공정·SCM — 초기 파일럿 조직이
안전환경 분야로 확정됨에 따라 v0.3에서 순서 변경)**

### 8.1 안전환경보건 (EHS) — 1차 · 파일럿

| 에이전트 | 시나리오 | 주요 입력/도구 |
|---|---|---|
| 안전 절차/MSDS Q&A | 화학물질·작업별 안전 절차, 보호구, 응급조치 질의응답. **공식 문서 인용 + 면책 고지 강제** (FR-GOV-03 도메인 안전 규칙) | 안전 절차서, MSDS, 사내 안전 규정 |
| 법규/규정 Q&A | 산업안전보건법·시행령·고시, KOSHA 가이드 기반 질의응답. **법규 개정 이력·시행일 인지** (FR-KNW-08) | 법규 원문, 개정 이력, 사내 준수 기준 |
| 위험성 평가 지원 | 작업 내용 입력 → 관련 위험요인·적용 법규·보호구·통제 대책 체크리스트 초안 생성 (승인 후 확정) | 위험성 평가 이력, 법규, 작업표준, HITL |
| 아차사고/사고 보고 지원 | 자연어 보고 → 표준 양식 구조화 + 유사 사고 사례 검색 + 재발 방지 대책 참고 제시. **인명 정보 PII 마스킹 필수** | 사고 보고서 DB, 사고 사례, 구조화 출력 |
| 안전 점검 브리핑 | 점검 일정/미조치 사항 조회 → 일일 안전 브리핑 자동 생성 (스케줄 트리거) | 점검 시스템 DB, 스케줄 실행 |
| 현장 사진 위험 식별 (P2) | 현장 사진 업로드 → 비전 모델로 불안전 상태·보호구 미착용 후보 식별 → 점검자 확인 | 비전 모델(FR-MDL-06), HITL |

> EHS는 오답의 파급이 큰 도메인이므로, 파일럿에서는 **① 인용 강제 ② 면책 고지 ③ 안전
> 관련 실행 도구 전면 승인제(HITL) ④ 골든셋에 레드팀 케이스 포함**을 배포 필수 조건으로
> 한다 (FR-GOV-03, FR-EVL-06).
...
| NFR-02 | 동시성 | 단일 GPU 서버(RTX PRO 6000 96GB 기준) 프로파일에서 동시 대화 세션 50+, K8s 프로파일에서 수평 확장 |
...
| NFR-09 | 다국어 | 한국어·영어 전 구간(문서 파싱·임베딩·검색·평가·Studio UI) 1급 지원. Studio UI는 i18n 구조로 개발(ko/en 기본 탑재). 기타 언어는 P2 |
...
| 로컬 모델 (한/영) | ✅ **Qwen3 계열** + 한국어 특화 모델(EXAONE 등) 병행 평가 | Llama 3.x | 한·영 및 함수호출 성능. RTX PRO 6000(96GB) 기준 32B급 FP8 또는 70B급 4bit 양자화 서빙 가능. 라이선스 확인 필수 (EXAONE은 상용 조건 검토) |
...
- 에이전트 컴포지션(Agent-as-Tool), 컨텍스트 윈도 관리, 생성 마법사, 휴먼 평가 모드,
  법규/규정 문서 관리, 배치 일괄 실행
- 품질·설비 템플릿
...
- 비전 모델 지원, 시맨틱 캐시/레이트 리밋, 법규 자동 수집
- 공정·SCM 템플릿
...
| HWP 파싱 품질 | ~~한국 제조 문서 다수가 HWP~~ → 비중 낮음 확인, 영향 축소 | P1로 후행. MVP는 PDF 변환 가이드로 우회, 파서 후보(hwp5, pyhwpx, 변환 서버)는 Phase 2에서 PoC |
...
### 확정 사항 (열린 질문 답변 반영 — v0.2)

| # | 질문 | 확정 답변 | 문서 반영 위치 |
|---|---|---|---|
| 1 | GPU 인프라 규모 | 연구소 내 RTX PRO 6000(96GB) — 개발/파일럿 표준 프로파일 | FR-DEP-01, NFR-02, 10장 로컬 모델 |
| 2 | 연동 대상 시스템 | 사내 MES/ERP/QMS/CMMS 솔루션. DB 뷰/API 방식을 **Agent 개발 시 선택**하여 바인딩 | FR-TOL-03 (연동 방식 선택형 설계) |
| 3 | HWP 문서 비중 | 많지 않음 → 네이티브 파서 P1로 후행, MVP는 PDF 변환 우회 | FR-KNW-01, 리스크 표, 로드맵 |
| 4 | 사내 IdP | 적용 대상(고객사/조직)에 따라 선택 필요 → **배포별 인증 프로파일**(Keycloak 단독 vs IdP 연동) 지원 | FR-GOV-01, 설계 원칙 8 |
| 5 | 외부 API 허용 | 일부 허용 가능하나 **고객사 상황에 따라 다름** → 배포 설정 팩에서 금지/부분 허용/허용 전환 | FR-MDL-02, 설계 원칙 8 |
| 6 | 초기 파일럿 조직 | **안전환경(EHS) 분야로 확정** → 롤아웃 순서 변경 (1차 EHS → 2차 품질·설비 → 3차 공정·SCM), 도메인 안전 규칙 P0 상향 | 8장, FR-GOV-03, 11장 로드맵 |
| 7 | 다국어 요구 | 한국어 + **영어 우선 지원** | NFR-09 (Studio UI i18n 포함) |

> 답변 과정에서 도출된 핵심 설계 방향: 본 플랫폼은 **복수 고객사/사이트에 개별 설치**되는
> 제품 성격을 가지므로, 환경 의존 요소(인증, 외부 API 정책, 커넥터, GPU 구성)를
> **배포 설정 팩(Deployment Profile)** 으로 분리한다 — 설계 원칙 8, FR-DEP-01 참조.

### 최종 리뷰 보완 사항 (v0.3 추가)

기능정의 전체 리뷰 결과, 다음 기능이 보완되었다.

| 추가/변경 | 내용 | 배경 |
|---|---|---|
| FR-AGT-08 (신규, P1) | 에이전트 컴포지션(Agent-as-Tool) | 도메인 에이전트 재사용 — EHS 위험성 평가가 법규 Q&A를 하위 호출하는 구조 |
| FR-AGT-09 (신규, P1) | 컨텍스트 윈도 관리 (요약 압축, 토큰 예산) | 로컬 모델의 제한된 컨텍스트에서 장기 대화 안정성 |
| FR-STD-08 (신규, P1) | 에이전트 생성 마법사 | 현업(비개발자) 온보딩 장벽 완화 |
| FR-KNW-08 (신규, P1) | 법규/규정 문서 관리 (시행일·개정 추적) | EHS 파일럿 — 법규 인용의 최신성·정확성 필수 |
| FR-MDL-06 (신규, P2) | 비전(멀티모달) 모델 지원 | 현장 사진 위험 식별 등 확장 대비 |
| FR-GOV-03 (보강) | 간접 프롬프트 인젝션 방어 (RAG 문서 유래), 도메인 안전 규칙 P1→P0 | 문서 기반 공격면 대응, EHS 파일럿 안전 요건 |
| FR-STD-05 (보강) | 배치 일괄 실행 채널 (P1) | 사고 보고 일괄 구조화 등 대량 처리 수요 |
| FR-EVL-02 (보강) | 휴먼 평가(블라인드 채점) 모드 (P1) | 안전 도메인은 자동 평가만으로 불충분 |
| FR-DEP-04 (신규, P2) | 레이트 리밋 + 시맨틱 캐시 | 단일 GPU 환경의 자원 보호 |

### 잔여 열린 질문

없음 — 전체 확정. 설계 단계 진행 가능.
